    ...
    return (
        <article className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
                ...
                <p className="font-work text-sm text-text-muted leading-[1.7] max-w-xl">{post.description}</p>
            </div>
            <div className="flex items-center gap-3">
                <span className="text-xs font-roboto text-text-muted">{post.author}</span>
                <span>|</span>
                <span className="text-xs font-roboto text-text-muted">{formatDate(post.createdAt)}</span>
            </div>
            <div className="w-full aspect-16/9 bg-gray-3 rounded-lg" />
            <div>
                <Button as="link" href={`/briefs/${post.slug}`} variant="primary" size="sm">
                    Read brief
                </Button>
            </div>
        </article>
